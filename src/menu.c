...
printf("║   8. Водитель с минимальным количеством поездок            ║\n");
printf("║   9. Автомобиль с максимальным пробегом                    ║\n");
printf("║  10. Отчет за период (все водители)                        ║\n");
printf("║  11. Отчет за период (конкретный водитель)                 ║\n");
printf("║   0. Выход                                                 ║\n");
printf("╚════════════════════════════════════════════════════════════╝\n");
}
    ...
    if (session->role == ROLE_ADMIN) {
        switch (choice) {
            ...
            case 10: {
                ...
                scanf("%s", end);
                calculate_and_save_all_drivers_earnings(db, start, end);
                break;
            }
            case 11: {
                char start[11], end[11], name[100];
                printf("Введите начальную дату (ГГГГ-ММ-ДД): ");
                scanf("%s", start);
                printf("Введите конечную дату (ГГГГ-ММ-ДД): ");
                scanf("%s", end);
                ...
                scanf("%s", name);
                calculate_and_save_driver_earnings(db, start, end, name);
                break;
            }
            case 0:
                return 0;
